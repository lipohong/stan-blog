...
export default function CollectionTreeRenderer({ collection, isAdminMode = false, noDataMessage, toBeContinuedMessage }: Readonly<CollectionTreeRendererProps>): ReactNode {
  const { t } = useCommonUtils();

  function printCollectionTree(collection: ICollectionDTO | null): ReactNode {
    if (!collection) {
      return <Typography color="textSecondary">{noDataMessage ?? t('collection-reading-page.tip.tba')}</Typography>;
    }
  ...
  function getDepthOfTree(treeNodes: ITagRelationshipDTO[]): number {
    if (!treeNodes || treeNodes.length === 0) {
      ...
      let result = 1;
      treeNodes?.forEach((n: ITagRelationshipDTO) => {
        const temp = 1 + getDepthOfTree(n.children);
        if (temp >= result) {
          result = temp;
        }
      });
...
const ContentLink = (props: { id: string; text: string; contentType: string; isAdminMode: boolean }): JSX.Element => {
...
const ToBeContinuedTip = (props: { message?: string }): JSX.Element => {
  ...
  return (
    <Grid
      ...
      xs={12}
    >
      <Typography color={'textSecondary'}>{props.message ?? t('collection-reading-page.tip.tba')}</Typography>
    </Grid>
  );
};
