...
export default function ContentAdminPage() {
  ...
  function loadData(option: ILoadDataOption): void {
    ...
    ContentAdminService.getContents(DEFAULT_PAGE_SIZE, pageToLoad - 1, option.keyword || undefined, option.status || undefined)
      .then(response => {
  ...
  const handleBan = useCallback(
    (id: number | string): void => {
      setContents(contents.map(c => (c.id === id ? { ...c, banned: true } : c)));
  ...
  const handleUnban = useCallback(
    (id: number | string): void => {
      setContents(contents.map(c => (c.id === id ? { ...c, banned: false } : c)));
      if (selectedContent && selectedContent.id === id) {
  ...
  const handleRecommend = useCallback(
    (id: number | string): void => {
      setContents(contents.map(c => (c.id === id ? { ...c, recommended: true } : c)));
      if (selectedContent && selectedContent.id === id) {
  ...
  const handleUnrecommend = useCallback(
    (id: number | string): void => {
      setContents(contents.map(c => (c.id === id ? { ...c, recommended: false } : c)));
      if (selectedContent && selectedContent.id === id) {
