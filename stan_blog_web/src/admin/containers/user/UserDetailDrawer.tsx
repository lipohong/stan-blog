import { Box, Typography, Avatar, Divider, Card, CardContent, Skeleton, useTheme, Button, Chip, FormControl, InputLabel, Select, MenuItem, SelectChangeEvent } from '@mui/material';
...
export default function UserDetailDrawer({ open, onClose, userId, onDelete }: IUserDetailDrawerProps) {
  ...
  const loadUserData = useCallback((): void => {
    ...
    UserService.getUser(userId)
      .then(response => {
        ...
        const rolesWithBasic = userRoles.includes('ROLE_BASIC') ? userRoles : [...userRoles, 'ROLE_BASIC'];
  ...
  const handleRoleChange = useCallback((event: SelectChangeEvent<string[]>): void => {
    ...
    const rolesWithBasic = newRoles.includes('ROLE_BASIC') ? newRoles : [...newRoles, 'ROLE_BASIC'];
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
        ...
        <Box sx={{ flexGrow: 1 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            {isVerified && <VerifiedIcon sx={{ color: 'success.main', fontSize: '1rem' }} />}
          </Box>
        </Box>
      </Box>
    );
  ...
  const hasRoleChanged = user?.roles && (selectedRoles.length !== user.roles.length || !selectedRoles.every(role => user.roles?.includes(role)));
    ...
    if (!user) {
      return (
        <Box sx={{ textAlign: 'center' }}>
          <Typography color="text.secondary">{t('user-detail.no-data')}</Typography>
        </Box>
      );
    }
      ...
      <>
        ...
        <Card sx={{ mb: 3, elevation: 0, border: `1px solid ${theme.palette.divider}` }}>
          <CardContent>
            ...
            {renderInfoItem(<EmailIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.email'), user.email, user.emailVerified)}

            {renderInfoItem(<PhoneIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.phone-number'), user.phoneNum)}

            {renderInfoItem(<LocationOnIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.address'), user.address)}

            {renderInfoItem(<WorkIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.profession'), user.profession)}

            {renderInfoItem(<LinkIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.blog'), user.blog)}

            {renderInfoItem(<AccessTimeIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.create-time'), formatDateTime(user.createTime))}

            {renderInfoItem(<UpdateIcon sx={{ fontSize: '1.2rem' }} />, t('user-detail.fields.update-time'), formatDateTime(user.updateTime))}
          </CardContent>
        </Card>
  ...
  const renderActionButtons = () => {
    if (!user || (user.emailVerified && !onDelete)) return null;
