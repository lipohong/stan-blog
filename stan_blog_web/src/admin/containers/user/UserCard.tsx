...
export default function UserCard({ user, onCardClick }: IUserCardProps) {
  ...
  const renderRoles = () => {
    ...
    const rolesToDisplay = user.roles.includes('ROLE_ADMIN') ? user.roles.filter(role => role !== 'ROLE_BASIC') : user.roles;
        ...
        <CardHeader
          ...
          title={
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              {user.emailVerified && <VerifiedIcon sx={{ color: 'success.main', fontSize: '1.1rem', ml: 1 }} />}
            </Box>
          }
          subheader={
            <Typography
