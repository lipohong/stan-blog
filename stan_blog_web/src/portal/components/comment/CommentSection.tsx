import { Box, Typography, Divider, CircularProgress, Alert, Pagination, Link } from '@mui/material';
...
export default function CommentSection({ content }: CommentSectionProps) {
  ...
  const loadComments = async (page: number = 1) => {
    ...
    setError(null);
    try {
      const response = await CommentService.getComments(content.id, content.contentType, page, pageSize);
