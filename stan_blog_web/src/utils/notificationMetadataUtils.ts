...
export function getCommentContent(notification: INotificationDTO): string | null {
  if (!notification.metadata || !['CONTENT_COMMENTED', 'COMMENT_REPLIED', 'COMMENT_DELETED', 'REPLY_DELETED'].includes(notification.notificationType)) {
    return null;
  }
...
export function isCommentReply(notification: INotificationDTO): boolean {
  return ['COMMENT_REPLIED', 'REPLY_DELETED'].includes(notification.notificationType) && getOriginalCommentContent(notification) !== null;
}
...
export function isI18nNotification(notification: INotificationDTO): boolean {
  return notification.title.startsWith('notifications.messages.') || notification.message.startsWith('notifications.messages.');
}
